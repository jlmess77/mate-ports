...
gs_auth_priv_init (void)
{
	/* We have nothing to do at init-time.
	   ...
	   If "/usr/local/etc/pam.d" exists and is a directory, but "/usr/local/etc/pam.d/xlock"
	...
	*/
	const char   dir [] = "/usr/local/etc/pam.d";
	const char  file [] = "/usr/local/etc/pam.d/" PAM_SERVICE_NAME;
